...
static int gen_vals_floating_point(int test_vals[], int min, int max, int test_range, int arg)
{
	int i = 0;
	int test_count = 0;
	...
	float _max_int = INT_MAX;
	float _min_int = INT_MIN;
	unsigned max_int = *(unsigned*)&_max_int;
	unsigned min_int = *(unsigned*)&_min_int;
	...
	for (i = 0; i < test_range; i++) {
		...
		/* Region around max int */
		test_vals[test_count++] = max_int - i; 
		test_vals[test_count++] = max_int + i; 
		
		/* Region around min int */
		test_vals[test_count++] = min_int - i; 
		test_vals[test_count++] = min_int + i; 
	}
...
static int gen_vals(int test_vals[], int min, int max, int test_range, int arg)
{
     ...
     */
    if ((min == 1 && max == 1)) { 
		return gen_vals_floating_point(test_vals, min, max, test_range, arg);
    }
...
static int test_function(test_ptr t) {
    ...
    int arg_test_range[3] = {0}; /* test range for each argument, 0s will be overwritten later */
    int i, a1, a2, a3;        
    int errors = 0;
